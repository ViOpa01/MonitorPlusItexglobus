...
int upay_print_proc( st_card_info *card_info )
{
	...
	UPrint_StrBold("Sales Slip", 1, 4, 1);//Centered large font print title,empty 4 lines
	...
	UPrint_SetFont(8, 2, 2);
	UPrint_Str("CARD NO.:", 1, 1);
	UPrint_SetFont(8, 3, 3);
	UPrint_StrBold(card_info->pan, 1, 4, 1); 

	UPrint_SetFont(8, 2, 2);
	UPrint_Str("TRANS TYPE", 1, 1);
	UPrint_SetFont(8, 3, 3);
	UPrint_Str(card_info->title, 1, 1 ); 
	...
	UPrint_Str("CARD HOLDER SIGNATURE", 1, 1);
	UPrint_Feed(36);
	UPrint_Str("I ACKNOWLEDGE	SATISFACTORY RECEIPT OF RELATIVE GOODS/SERVICES", 1, 1);
	UPrint_Feed(24);
	UPrint_Str("---X---X---X---X---X--X--X--X--X--X--X--X--X--", 1, 1);
	UPrint_Feed(108);
