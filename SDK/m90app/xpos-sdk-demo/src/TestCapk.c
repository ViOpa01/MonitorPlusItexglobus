/**
 * File: TestCapk.c 
 * ----------------
 * Implements TestCapk.h's interface.
 */

#include <stdio.h>

#include "TestCapk.h"
#include "libapi_xpos/inc/def.h"
#include "emvapi/inc/emv_api.h"


#if 1

/*
* These capk are not working
*
CAPUBLICKEY capk_visa_t92 = {
	{0xA0, 0x00, 0x00, 0x00, 0x03},
	0x5C,
	0x01,
	0x01,
	176,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x42, 0x9C, 0x95, 0x4A, 0x38, 0x59, 0xCE, 0xF9, 0x12, 0x95, 0xF6, 0x63, 0xC9, 0x63, 0xE5, 0x82, 0xED, 0x6E, 0xB2, 0x53},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_visa_t94 = {
	{0xA0, 0x00, 0x00, 0x00, 0x03},
	0x5E,
	0x01,
	0x01,
	248,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0xC4, 0xA3, 0xC4, 0x3C, 0xCF, 0x87, 0x32, 0x7D, 0x13, 0x6B, 0x80, 0x41, 0x60, 0xE4, 0x7D, 0x43, 0xB6, 0x0E, 0x6E, 0x0F},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_visa_t95 = {
	{0xA0, 0x00, 0x00, 0x00, 0x03},
	0x5F,
	0x01,
	0x01,
	144,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0xEE, 0x15, 0x11, 0xCE, 0xC7, 0x10, 0x20, 0xA9, 0xB9, 0x04, 0x43, 0xB3, 0x7B, 0x1D, 0x5F, 0x6E, 0x70, 0x30, 0x30, 0xF6},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_visa_t99 = {
	{0xA0, 0x00, 0x00, 0x00, 0x03},
	0x63,
	0x01,
	0x01,
	128,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x4A, 0xBF, 0xFD, 0x6B, 0x1C, 0x51, 0x21, 0x2D, 0x05, 0x55, 0x2E, 0x43, 0x1C, 0x5B, 0x17, 0x00, 0x7D, 0x2F, 0x5E, 0x6D},
	{0x99, 0x12, 0x31}
};
*/

CAPUBLICKEY capk_master_t00 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0x00,
	0x01,
	0x01,
	160,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0xEC, 0x0A, 0x59, 0xD3, 0x5D, 0x19, 0xF0, 0x31, 0xE9, 0xE8, 0xCB, 0xEC, 0x56, 0xDB, 0x80, 0xE2, 0x2B, 0x1D, 0xE1, 0x30},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_master_t01 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0x01,
	0x01,
	0x01,
	128,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x8C, 0x05, 0xA6, 0x41, 0x27, 0x48, 0x5B, 0x92, 0x3C, 0x94, 0xB6, 0x3D, 0x26, 0x4A, 0xF0, 0xBF, 0x85, 0xCB, 0x45, 0xD9},
	{0x99, 0x12, 0x31}
};
/*
* This capk is not working
CAPUBLICKEY capk_master_t02 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0x02,
	0x01,
	0x01,
	192,
	{},
	1,
	{0x00, 0x00, 0x03},
	{},
	{0x99, 0x12, 0x31}
};
*/

/*
CAPUBLICKEY capk_master_t05 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0x05,
	0x01,
	0x01,
	128,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x53, 0xD0, 0x49, 0x03, 0xB4, 0x96, 0xF5, 0x95, 0x44, 0xA8, 0x43, 0x09, 0xAF, 0x16, 0x92, 0x51, 0xF2, 0x89, 0x68, 0x74},
	{0x99, 0x12, 0x31}
};
*/


CAPUBLICKEY capk_master_t239 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xEF,
	0x01,
	0x01,
	248,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x21, 0x76, 0x6E, 0xBB, 0x0E, 0xE1, 0x22, 0xAF, 0xB6, 0x5D, 0x78, 0x45, 0xB7, 0x3D, 0xB4, 0x6B, 0xAB, 0x65, 0x42, 0x7A},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_master_t241 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xF1,
	0x01,
	0x01,
	176,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0xD8, 0xE6, 0x8D, 0xA1, 0x67, 0xAB, 0x5A, 0x85, 0xD8, 0xC3, 0xD5, 0x5E, 0xCB, 0x9B, 0x05, 0x17, 0xA1, 0xA5, 0xB4, 0xBB},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_master_t243 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xF3,
	0x01,
	0x01,
	144,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0xA6, 0x9A, 0xC7, 0x60, 0x3D, 0xAF, 0x56, 0x6E, 0x97, 0x2D, 0xED, 0xC2, 0xCB, 0x43, 0x3E, 0x07, 0xE8, 0xB0, 0x1A, 0x9A},
	{0x99, 0x12, 0x31}
};

/*
* This capk is not working
CAPUBLICKEY capk_master_t245 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xF5,
	0x01,
	0x01,
	248,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0xC2, 0x23, 0x98, 0x04, 0xC8, 0x09, 0x81, 0x70, 0xBE, 0x52, 0xD6, 0xD5, 0xD4, 0x15, 0x9E, 0x81, 0xCE, 0x84, 0x66, 0xBF},
	{0x99, 0x12, 0x31}
};
*/

CAPUBLICKEY capk_master_t246 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xF6,
	0x01,
	0x01,
	224,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x50, 0x29, 0x09, 0xED, 0x54, 0x5E, 0x3C, 0x8D, 0xBD, 0x00, 0xEA, 0x58, 0x2D, 0x06, 0x17, 0xFE, 0xE9, 0xF6, 0xF6, 0x84},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_master_t248 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xF8,
	0x01,
	0x01,
	128,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0xF0, 0x6E, 0xCC, 0x6D, 0x2A, 0xAE, 0xBF, 0x25, 0x9B, 0x7E, 0x75, 0x5A, 0x38, 0xD9, 0xA9, 0xB2, 0x4E, 0x2F, 0xF3, 0xDD},
	{0x99, 0x12, 0x31}
};

/*
* This capk is not working
CAPUBLICKEY capk_master_t249 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xF9,
	0x01,
	0x01,
	192,
	{},
	1,
	{0x00, 0x00, 0x03},
	{},
	{0x99, 0x12, 0x31}
};
*/

CAPUBLICKEY capk_master_t250 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xFA,
	0x01,
	0x01,
	144,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x5B, 0xED, 0x40, 0x68, 0xD9, 0x6E, 0xA1, 0x6D, 0x2D, 0x77, 0xE0, 0x3D, 0x60, 0x36, 0xFC, 0x7A, 0x16, 0x0E, 0xA9, 0x9C},
	{0x99, 0x12, 0x31}
};

CAPUBLICKEY capk_master_t254 = {
	{0xA0, 0x00, 0x00, 0x00, 0x04},
	0xFE,
	0x01,
	0x01,
	128,
	{},
	1,
	{0x00, 0x00, 0x03},
	{0x9A, 0x29, 0x5B, 0x05, 0xFB, 0x39, 0x0E, 0xF7, 0x92, 0x3F, 0x57, 0x61, 0x8A, 0x9F, 0xDA, 0x29, 0x41, 0xFC, 0x34, 0xE0},
	{0x99, 0x12, 0x31}
};


struct{
	const char * label;
	CAPUBLICKEY * keyValue;
}AllTestEmvKey[]={
/*
* These capk keys are not working
{"capk_visa_t92", &capk_visa_t92},
{"capk_visa_t94", &capk_visa_t94},
{"capk_visa_t95", &capk_visa_t95},
{"capk_visa_t99", &capk_visa_t99},

{"capk_master_t02", &capk_master_t02},
{"capk_master_t245", &capk_master_t245},
{"capk_master_t249", &capk_master_t249},

*/

{"capk_master_t00", &capk_master_t00},
{"capk_master_t01", &capk_master_t01},

//{"capk_master_t05", &capk_master_t05},
{"capk_master_t239", &capk_master_t239},
{"capk_master_t241", &capk_master_t241},
{"capk_master_t243", &capk_master_t243},

{"capk_master_t246", &capk_master_t246},
{"capk_master_t248", &capk_master_t248},
{"capk_master_t250", &capk_master_t250},
{"capk_master_t254", &capk_master_t254},
};

#endif 



#define ALL_LIVE_CAPK_SIZE sizeof(AllTestEmvKey) / sizeof(AllTestEmvKey[0])

void injectTestCapks(void)
{
	int i;
	int ret = -1;

    
    const int size = ALL_LIVE_CAPK_SIZE;

	for(i = 0; i < size; i++)
	{
        
        if (ret = EMV_PrmSetCAPK(AllTestEmvKey[i].keyValue)) {
            fprintf(stderr, "Error injecting test key %s\n", AllTestEmvKey[i].label);
        }
        
		printf("Result ret : %d\n", ret);
	}
    
}


